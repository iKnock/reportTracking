...
class Kebele {
    ...
    featchKebeleInWoreda = function (woredaName, callback) {
        dbConnection.getConnection(function (connection, conError) {
            if (conError != null) {
                const conErr = new AppExceptions(conError.code, conError.message);
                callback(null, conErr);
            } else {
                connection.query('SELECT * FROM tbl_kebele where W_NAME = ?',[woredaName], function (error, results, fields) {
                    try {
                        var kebeles = [];
                        for (var i in results) {
                            if (results.hasOwnProperty(i)) {
    ...
    featchKebeleInZone = function (zoneName, callback) {
        dbConnection.getConnection(function (connection, conError) {
            if (conError != null) {
                const conErr = new AppExceptions(conError.code, conError.message);
                callback(null, conErr);
            } else {
                connection.query('SELECT * FROM tbl_kebele where Z_NAME = ?',[zoneName], function (error, results, fields) {
                    try {
                        var kebeles = [];
                        for (var i in results) {
                            if (results.hasOwnProperty(i)) {
    ...
    featchKebeleInRegion = function (regionName, callback) {
        dbConnection.getConnection(function (connection, conError) {
            if (conError != null) {
                const conErr = new AppExceptions(conError.code, conError.message);
                callback(null, conErr);
            } else {
                connection.query('SELECT * FROM tbl_kebele where R_NAME = ?',[regionName], function (error, results, fields) {
                    try {
                        var kebeles = [];
                        for (var i in results) {
                            if (results.hasOwnProperty(i)) {
